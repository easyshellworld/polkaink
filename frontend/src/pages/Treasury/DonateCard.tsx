...
export function DonateCard({ rewardPoolBalance }: { rewardPoolBalance: bigint }) {
  ...
  const handleDonate = async () => {
    ...
    setSubmitting(true);
    try {
      addNotification({ id: nid, type: 'pending', message: 'Submitting donation...' });
      ...
      updateNotification(nid, { type: 'success', message: 'Donation submitted.' });
      setAmount('');
    } catch (err) {
  ...
  return (
    <Card className="mb-6">
      <h2 className="text-sm font-semibold mb-3">Donate to Reward Pool</h2>

      {isPaused && (
        <div className="mb-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-700 font-medium">
          Rewards are currently paused (pool balance &lt; 50 PAS).
        </div>
      )}
